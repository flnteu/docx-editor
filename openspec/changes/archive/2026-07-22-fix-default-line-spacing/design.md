...
2. `buildBoxTree.ts` skips spacing.line when `pmAttrs.lineSpacing == null`
3. `paragraphLayout.ts` hits the else fallback → uses `singleLineBase × 1.15`
...
2. `buildBoxTree.ts` converts to `spacing.line = 276/240 = 1.15, lineUnit = 'multiplier'`
3. `paragraphLayout.ts` uses multiplier branch → `singleLineBase × 1.15`
...
- `paragraphLayout.ts` (layout engine — line breaking + height calculation)
- `textMetrics.ts` (font metrics — lineHeight field)
