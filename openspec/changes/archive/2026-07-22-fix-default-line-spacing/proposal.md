...
- Change the fallback line spacing multiplier from 1.15 to 1.0 in `paragraphLayout.ts` when no line spacing is specified
- Change the corresponding fallback in `textMetrics.ts` (`DEFAULT_LINE_HEIGHT_MULTIPLIER`)
