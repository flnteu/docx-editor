...
```typescript
import pl from '@docx-editor.dev/react/i18n/pl.json';
```
...
- **AND** the file SHALL be usable immediately via `import de from '@docx-editor.dev/react/i18n/de.json'`
...
The package `package.json` SHALL include exports entries for locale JSON files so they can be imported by consumers using the `@docx-editor.dev/react/i18n/{lang}` path.

#### Scenario: Consumer imports a locale

- **WHEN** a consumer writes `import pl from '@docx-editor.dev/react/i18n/pl.json'`
