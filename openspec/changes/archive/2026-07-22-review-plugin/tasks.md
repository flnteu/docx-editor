...
- [x] 1.1 Create `packages/agent-use/` with `package.json` (`@docx-editor.dev/agents`, BSL-1.1, peer dep on `@docx-editor.dev/core`), `tsconfig.json`, `tsup.config.ts` with two entry points (`src/index.ts`, `src/bridge.ts`)
