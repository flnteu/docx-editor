...
Monorepo: `packages/core` (`@docx-editor.dev/core`, MIT), `packages/react`, `packages/vue`. Root workspace `"packages/*"`. Build: tsup. New package: `packages/agent-use` → `@docx-editor.dev/agents` (BSL-1.1).
...
`packages/agent-use/` with `@docx-editor.dev/core` as peer dep. Published on public npm under BSL-1.1.
