...
- [x] 1.3 Create `packages/core/package.json` with name `@docx-editor.dev/core`, dependencies (prosemirror-\*, jszip, pizzip, xml-js, docxtemplater, clsx), and exports config
- [x] 1.4 Create `packages/react/package.json` with name `@docx-editor.dev/react`, dependency on `@docx-editor.dev/core`, peerDependencies (react, react-dom), and `@radix-ui/react-select`
- [x] 1.5 Create `packages/vue/package.json` with name `@docx-editor.dev/vue`, dependency on `@docx-editor.dev/core`, peerDependency on `vue`, and a placeholder `src/index.ts`
...
- [x] 5.3 Export `EditorPluginCore`, `PluginPanelProps`, `PanelConfig`, `RenderedDomContext` from `@docx-editor.dev/core`
...
- [x] 5.8 Export `ReactEditorPlugin` from `@docx-editor.dev/react` and `VueEditorPlugin` from `@docx-editor.dev/vue`
...
- [x] 10.1 Verify `@docx-editor.dev/react` main export includes all previously exported symbols
- [x] 10.2 Verify subpath exports (`/core`, `/headless`, `/react`, `/ui`, `/core-plugins`, `/mcp`) still work from the React package
- [x] 10.3 Verify `@docx-editor.dev/core` can be imported and used without React installed
