...
- **BREAKING**: Split the single `@docx-editor.dev/react` package into a Bun workspaces monorepo with separate packages
- New `@docx-editor.dev/core` package containing all framework-agnostic code: `src/docx/`, `src/types/`, `src/prosemirror/`, `src/pagination-model/`, `src/painter-model/`, `src/flow-model/`, `src/core-plugins/`, `src/utils/` (non-React parts), `src/agent/`, `src/mcp/`
- `@docx-editor.dev/react` becomes a thin React UI package depending on `@docx-editor.dev/core`, containing: `src/components/`, `src/hooks/`, `src/plugin-api/`, `src/plugins/`, `src/paged-editor/`
- Remove React type leaks from core code paths (e.g., `CSSProperties` imports in utils)
- Split `EditorPlugin` interface into a framework-agnostic core (`EditorPluginCore` in `@docx-editor.dev/core`) and framework-specific adapters (`ReactEditorPlugin` in React package, `VueEditorPlugin` in Vue package)
- Existing `@docx-editor.dev/react` keeps its npm name and download stats — existing users continue using it as before
...
- `core-package-api`: Public API surface for `@docx-editor.dev/core` — exports for DOCX parsing/serialization, ProseMirror schema/extensions/commands, layout engine, document model types, and headless editor creation
...
- **npm**: New `@docx-editor.dev/core` package published; `@docx-editor.dev/react` gains `@docx-editor.dev/core` as dependency; `@docx-editor.dev/vue` scaffolded for community contribution
...
- **Downstream**: Vue/Svelte/vanilla consumers can depend on `@docx-editor.dev/core` directly with zero React overhead
