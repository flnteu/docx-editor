...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@docx-editor.dev/core`.
...
The `packages/react/` directory SHALL contain all React-dependent source code with its own `package.json` named `@docx-editor.dev/react`.
...
- **THEN** `@docx-editor.dev/core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@docx-editor.dev/core` (not bundle it)
...
- **THEN** tests SHALL import from both `@docx-editor.dev/core` and `@docx-editor.dev/react`
...
- **AND** the app SHALL render a functional DOCX editor using `@docx-editor.dev/vue` and `@docx-editor.dev/core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@docx-editor.dev/vue` with a dependency on `@docx-editor.dev/core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@docx-editor.dev/vue`
- **AND** `@docx-editor.dev/core` SHALL be listed in dependencies
