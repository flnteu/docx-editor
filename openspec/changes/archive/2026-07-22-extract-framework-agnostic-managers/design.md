...
This change follows `extract-core-monorepo`, which splits the codebase into `@docx-editor.dev/core` and `@docx-editor.dev/react` (React). After that split, the package boundary is clean but the React package still contains ~50% framework-agnostic business logic embedded in React components and hooks. Specifically:
...
A Vue contributor would need to reimplement all of this logic. The goal is to extract the state machines and coordination logic into framework-agnostic classes in `@docx-editor.dev/core`.

## Goals / Non-Goals

**Goals:**

- Extract framework-agnostic state machines and coordination logic from React components into plain TypeScript classes in `@docx-editor.dev/core`
...
- Changing any public API of `@docx-editor.dev/react`
...
class LayoutCoordinator {
  ...
  getSnapshot() {
    return { layout: this.layout, selectionGeometry: this.selectionGeometry, ... };
  }
}
...
### 4. Keep managers in `@docx-editor.dev/core` as plain classes

**Decision:** All manager/coordinator classes live in `packages/core/src/managers/` and are exported from `@docx-editor.dev/core`.
